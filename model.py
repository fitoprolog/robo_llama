...
class ConditionProjector(nn.Module):
    """Projects the conditioning tensor to the model dimension and prepares it for fusion"""
    def __init__(self, condition_dim: int, model_dim: int):
        super().__init__()
        self.projector = nn.Linear(condition_dim, model_dim, bias=False)
        self.norm = RMSNorm(model_dim, eps=1e-5)
        
    def forward(self, condition: torch.Tensor) -> torch.Tensor:
        """
        Projects the condition vector to the model dimension
        
        Args:
            condition: Shape [batch_size, condition_dim]
            
        Returns:
            Projected condition: Shape [batch_size, 1, model_dim]
        """
        # Project to model dimension
        projected = self.projector(condition)
        # Normalize
...
class Transformer(nn.Module):
    ...
    def __init__(self, params: ModelArgs):
        ...
        self.condition_projector = ConditionProjector(
            ...
            params.condition_proj_dim or params.dim
        )
    ...
    def forward(self, tokens: torch.Tensor, condition: Optional[torch.Tensor] = None, targets: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if condition is not None:
            # Project condition to the model dimension
